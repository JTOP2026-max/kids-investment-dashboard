const CACHE='kids-investment-pwa-v1';
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.startsWith('/api/quotes'))return;
  ...
  if(url.origin===location.origin){event.respondWith(caches.match(req).then(cached=>cached||fetch(req).then(res=>{const copy=res.clone();caches.open(CACHE).then(c=>c.put(req,copy));return res;})));}
});